@testset "ReweightedL1" begin
    @testset "Basic sparse recovery" begin
        Random.seed!(1111)
        # Use smaller problem size since ReweightedL1 uses convex optimization
        n, p, k = 20, 50, 3
        A, x_true, b = gaussian_sensing(n, p, k)

        x_recovered = ReweightedL1(A, b; maxiter=5, epsilon=0.01)
    ...
    @testset "Output properties" begin
        ...
        x_recovered = ReweightedL1(A, b; maxiter=3)

        @test length(x_recovered) == p
        @test eltype(x_recovered) <: Real

        # Residual check - ReweightedL1 enforces equality constraint
    ...
    @testset "Sparsity promotion" begin
        ...
        x_recovered = ReweightedL1(A, b; maxiter=5, epsilon=0.05)
    ...
    @testset "Epsilon thresholding" begin
        ...
        epsilon = 0.1
        x_recovered = ReweightedL1(A, b; maxiter=3, epsilon=epsilon)
